              ...
              {reminders.map(r => (
                <div key={r.id} className="flex items-center justify-between gap-2 p-1.5 hover:bg-slate-50/50 rounded-lg group transition-colors">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    />
                    <span className={`text-[11px] truncate leading-normal ${r.done ? "text-slate-400 line-through" : "text-slate-700"}`}>
                      {r.text}
                    </span>
                  </div>
          ...
          <div key={widget.id} className="p-4 bg-white rounded-xl border border-slate-200 shadow-2xs space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              <div className="p-2 bg-slate-50 rounded border border-slate-100">
                <span className="block text-[8px] text-slate-400 uppercase font-bold tracking-wider">RESPON TIME</span>
                <span className="block text-xs font-bold text-slate-800 font-mono mt-0.5">~1.1s (Cepat)</span>
              </div>
              <div className="p-2 bg-slate-50 rounded border border-slate-100">
                <span className="block text-[8px] text-slate-400 uppercase font-bold tracking-wider">UPTIME</span>
                <span className="block text-xs font-bold text-slate-800 font-mono mt-0.5">99.98%</span>
              </div>
            </div>
            <p className="text-[9px] text-slate-400 leading-snug italic text-center">
